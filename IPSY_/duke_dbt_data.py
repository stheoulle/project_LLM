...
def safe_dcmread(fp: Union[str, "os.PathLike", BinaryIO]) -> tuple[pydicom.Dataset, np.ndarray]:
    """
    Lit un fichier DICOM et décompresse automatiquement avec un handler disponible.
    Supporte JPEG 2000 lossless.
    """
    ...
    try:
        ...
        print("Décompression automatique échouée :", e)
        print("Handlers disponibles :", pydicom.config.image_handlers)
        raise RuntimeError(
            "Impossible de décompresser ce DICOM. "
            "Vérifie que pylibjpeg>=2.0 et pylibjpeg-openjpeg>=2.0 sont installés."
        )
...
def dcmread_image(
    ...
    """
    Lit le pixel_array depuis un fichier DICOM DBT de manière robuste.
    Retourne uniquement l'image en numpy.ndarray.
    """
    ds, pixel_array = safe_dcmread(fp)
    
    # Si tu as besoin, tu peux aussi utiliser `ds` pour récupérer des métadonnées
...
def read_boxes(
    boxes_fp: Union[str, Path, IO], filepaths_fp: Union[str, Path, IO]
) -> pd.DataFrame:
...
def evaluate(
    labels_fp: Union[str, Path, IO],
    boxes_fp: Union[str, Path, IO],
    predictions_fp: Union[str, Path, IO],
) -> Dict[str, float]:
